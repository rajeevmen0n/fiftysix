...
/**
 * Rules §9, design §7.1: each seat's complete holding for the redeal check.
 * In 28 the face-down card is not in its owner's hand but still counts as
 * part of that owner's eight cards, for both hand points and team Jacks.
 */
export function completeHoldings(
  ...
  return hands.map((hand, seat) =>
    faceDown !== null && faceDown.owner === seat
      ? [...hand, faceDown.card]
      : [...hand],
  );
}
